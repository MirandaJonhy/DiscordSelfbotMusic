...
module.exports = {
  info: {
    name: "queue",
    description: "Mostrar a queue do servidor.",
    usage: "",
  ...
  const permissions = message.channel.permissionsFor(message.client.user);
    if (!permissions.has(["MANAGE_MESSAGES", "ADD_REACTIONS"]))
      return sendError("Faltando permissões para adicionar reações",message.channel);

    const queue = message.client.queue.get(message.guild.id);
    if (!queue) return sendError("Não há nada tocando na queue.",message.channel)
...
function generateQueueEmbed(message, queue) {
  ...
  for (let i = 0; i < queue.length; i += 10) {
    ...
    const embed = new MessageEmbed()
     .setAuthor("Queue do Servidor", "https://raw.githubusercontent.com/SudhanPlayz/Discord-MusicBot/master/assets/Music.gif")
    //.setThumbnail(message.guild.iconURL())
    .setColor("BLUE")
    .setDescription(`${info}`)
    .addField("Tocando:", `[${queue[0].title}](${queue[0].url})`, true)
